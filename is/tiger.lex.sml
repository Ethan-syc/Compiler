...
STRING | COMMENT | FORMATTING | INITIAL
...
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (commentDepth := !commentDepth + 1; gotoState("INITIAL", "COMMENT"); YYBEGIN COMMENT; continue()))
fun yyAction45 (strm, lastMatch : yymatch) = (yystrm := strm;
      (err(yypos, "unexpected comment terminator"); continue()))
...
fun yyAction49 (strm, lastMatch : yymatch) = (yystrm := strm;
      (gotoState("INITIAL", "STRING"); YYBEGIN STRING; currString := ""; stringStartPos := yypos; stringOpenState := 1; continue()))
fun yyAction50 (strm, lastMatch : yymatch) = (yystrm := strm;
      (currString := !currString ^ "\n"; continue()))
fun yyAction51 (strm, lastMatch : yymatch) = (yystrm := strm;
      (currString := !currString ^ "\t"; continue()))
fun yyAction52 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (currString := !currString ^ Char.toString (valOf(Char.fromString yytext)); continue())
      end
...
fun yyAction56 (strm, lastMatch : yymatch) = (yystrm := strm;
      (err(yypos, "EOL while scanning string literal, string literal can't contain new line character"); newLine(yypos); continue()))
fun yyAction57 (strm, lastMatch : yymatch) = (yystrm := strm;
      (err(yypos, "EOL while scanning string literal, string literal can't contain tab character"); continue()))
fun yyAction58 (strm, lastMatch : yymatch) = (yystrm := strm;
      (gotoState("STRING", "FORMATTING"); YYBEGIN FORMATTING; continue()))
fun yyAction59 (strm, lastMatch : yymatch) = (yystrm := strm;
      (gotoState("STRING", "INITIAL"); YYBEGIN INITIAL; stringOpenState := 0; Tokens.STRING(!currString, !stringStartPos, yypos)))
fun yyAction60 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (currString := !currString ^ yytext; continue())
      end
fun yyAction61 (strm, lastMatch : yymatch) = (yystrm := strm;
      (gotoState("FORMATTING", "STRING"); YYBEGIN STRING; continue()))
...
fun yyAction64 (strm, lastMatch : yymatch) = (yystrm := strm;
      (err(yypos, "expecting only white space in formatting string block"); continue()))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction1(strm, yyNO_MATCH)
                      else yyQ63(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction1(strm, yyNO_MATCH)
                  else yyQ63(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
            else if inp = #"`"
              ...
              then if inp = #"_"
                  then yyQ63(strm', yyMATCH(strm, yyAction1, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #"["
              ...
              then if inp = #":"
                  ...
                  then if inp <= #"/"
                      then yyAction0(strm, yyNO_MATCH)
                      else yyQ63(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
                else if inp <= #"@"
                  then yyAction0(strm, yyNO_MATCH)
                  else yyQ63(strm', yyMATCH(strm, yyAction0, yyNO_MATCH))
            else if inp = #"`"
       ...
       of NONE => yyAction41(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then if inp = #"`"
                  then yyAction41(strm, yyNO_MATCH)
                  else yyQ101(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
            else if inp <= #"z"
              then yyQ63(strm', yyMATCH(strm, yyAction41, yyNO_MATCH))
              else yyAction41(strm, yyNO_MATCH)
      (* end case *))
...
fun yyQ40 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction38(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"="
              then yyQ110(strm', yyMATCH(strm, yyAction38, yyNO_MATCH))
...
fun yyQ38 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction26(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ112(strm', yyMATCH(strm, yyAction26, yyNO_MATCH))
...
in
  (case (!(yyss))
   of STRING => yyQ0(!(yystrm), yyNO_MATCH)
    ...
    | FORMATTING => yyQ2(!(yystrm), yyNO_MATCH)
